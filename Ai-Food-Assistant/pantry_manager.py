...
def consume_product(product_name, quantity):
    """طرح الكمية المستهلكة من المخزون"""
    data = load_pantry()

    if product_name in data:
        if data[product_name]["current_quantity"] >= quantity:
            data[product_name]["current_quantity"] -= quantity
            save_pantry(data)
